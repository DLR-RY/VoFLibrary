...
void setAlpha(const fvMesh& mesh, const dictionary& initAlphaFieldDict, volScalarField& alpha1)
{
    ...
    (
           initAlphaFieldDict.lookup("function"),
           initAlphaFieldDict
    );
    ...
    forAll(alpha1,cellI)
    {
        ...
        {
            alpha1[cellI]= cutCell.VolumeOfFluid();
        }
...
//    #include "createMesh.H"
    ...
    const bool curvedSurface = initAlphaFieldDict.lookupOrDefault<bool>("curvedSurface",false);
    ...
    forAll(alpha1,cellI)
    {
        ...
        {
            alpha1[cellI]= cutCell.VolumeOfFluid();
            vector faceArea = cutCell.faceArea();

            if (writeVTK && cellStatus == 0 && (mag(faceArea) >= 1e-14))
            {
    ...
    if(curvedSurface && writeVTK)
    {
        // loop over all isosurface
        DynamicList<point> dynPoints(mesh.nCells());
        DynamicList<triFace> dyntriFaces(mesh.nCells());
        DynamicList<triFace> dynlocalTriFaces(200);

        label numberofPoints = 0;
        forAll(surface,i)
        {
            dynPoints.append(surface[i].points());

            //triFaceList localtriFaces(surface[i].surfFaces().size());
            dynlocalTriFaces.setSize(surface[i].surfFaces().size());
            forAll(surface[i].surfFaces(),triI)
            {
                forAll(surface[i].surfFaces()[triI],triPI)
                {
                    dynlocalTriFaces[triI][triPI] = surface[i].surfFaces()[triI][triPI] + numberofPoints;

                }
            }
            dyntriFaces.append(dynlocalTriFaces);
            numberofPoints += surface[i].points().size();
        }
        pointField surfacePoints(dynPoints);
        triFaceList triFaces(dyntriFaces);

        fileName filesurfCurved(word("initAlphaCurvedSurfaces.vtk") );
        triSurface interface(triFaces,surfacePoints);

        interface.write(filesurfCurved);
    }
