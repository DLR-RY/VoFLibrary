...
//	Info<< "Reading field U\n" << endl;
    ...
    Info << "min(magU).value() " << min(magU).value() << endl;
    Info << "max(magU).value() " << max(magU).value() << endl;
    if((max(magU).value()-min(magU).value()) >= SMALL )
    {
        U0 = Foam::vector(0,0,0);
    }
    ...
    forAll(timeDirs, timeI)
    {
        ...
        else if ( surfType == "ellipsoid" )
        {
            const scalar a(readScalar(isoSurfDict.lookup("a")));
            const scalar b(readScalar(isoSurfDict.lookup("b")));
            const scalar c(readScalar(isoSurfDict.lookup("c")));


                //f = -mag(mesh.points() - centre);
            scalarField x = mesh.points().component(0) - centre.component(0);
            scalarField y = mesh.points().component(1) - centre.component(1);
            scalarField z = mesh.points().component(2) - centre.component(2);

            f = -(sqr(x/a)+sqr(y/b)+sqr(z/c));
            f0 = -1.0;
        }
...
Info << "set alpha1Exact" << endl;
            alpha1Exact = 0;
            isoCutCell icc(mesh, f);
            icc.volumeOfFluid(alpha1Exact, f0);
//Info << "f" << f  << endl;
        }
Info << "alpha1Exact" << sum(mag(alpha1Exact)*mesh.V())  << endl;
Info << "diff alpha1Exact" << sum(mag(alpha1Exact-alpha1)*mesh.V())  << endl;
...
//        isoCutCell icc(mesh, f);
        ...
        scalar length = average(mag(mesh.delta())()).value();
...
//		cout << setprecision(2) << E1 << " "
		cout << setw(w) << setprecision(3) << E1/V0 << " "
		     << setw(w) << setprecision(3) << (sum(mesh.V()*alpha1).value()-V0)/V0 << " "
		     << setw(w) << setprecision(3) << min(alpha1).value() << " "
			 << setw(w) << setprecision(3) << 1-max(alpha1).value() << " "
             << setw(w) << setprecision(4) << length << " "
             << setw(w) << setprecision(4) << E1 << " "
		//	 << setw(w) << setprecision(3) << (dV-dVExact)/(dVExact+SMALL) << " "
//			 << setprecision(2) << dV << " "
