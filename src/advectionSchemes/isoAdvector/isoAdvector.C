...
//    centre_ = surf_->centre();
//    normal_ = surf_->normal();

    // Loop through cells
    forAll(alpha1_, celli)
    {
        if (surf_->interfaceCell()[celli])
        {
...
)
{
    ...
    forAll(alpha1, celli)
    {
        if (checkBounding_[celli])
        {
            const scalar& Vi = mesh_.V()[celli];
            scalar alpha1New = (alpha1[celli]*rdT + Su[celli] - netFlux(dVf, celli)/Vi*rdT)/(rdT-Sp[celli]);
            scalar alphaUndershoot = mag(alpha1New)*pos(-alpha1New); // pos is with zero
	...
	//                    downwind faces
		        ...
		        firstLoop = false;
		        alpha1New = (alpha1[celli]*rdT + Su[celli] - netFlux(dVf, celli)/Vi*rdT)/(rdT-Sp[celli]);
        //	Info << "alpha1New " << alpha1New << endl;
		        alphaUndershoot =  mag(alpha1New)*pos(-alpha1New);
        //	Info << "alphaUndershoot " << alphaUndershoot << endl;
...
void Foam::advection::isoAdvector::calcFaceFlux()
{
    ...
    forAll(dVf_,faceI) // all are internal
    {
        ...
        const label nei = neighbour[faceI];
        {
            ...
            scalar weight = pos(phi_[faceI]);
            if(weight == 1 && surf_().interfaceCell()[own]) // mag(surf_->normal()[own])!= 0)
            {
                // take own value
    ...
    forAll(dVfb,patchI) // loop patches
    {
        ...
        {
            scalar weight = pos(phib[patchI][i]);
    ...
    surfaceCellFacesOnProcPatches_(0)
{
    ...
    surf_ = reconstructionSchemes::New(alpha1,phi,U,modelDict());
